...
def process_entire_client_lifecycle(client, sorted_prompt_files, shared_auth_path):
    ...
    client_storage = SCRIPT_DIR / f".storage_fast_{client}.json"
        ...
        for prompt_file in sorted_prompt_files:
            ...
            try:
                if "ask" in prompt_file.name.lower():
                    run_cmd(["notebooklm", "--storage", str(client_storage), "source", "add-research", "--prompt-file", str(temp_prompt.resolve()), "--no-wait"])
                    
                    max_wait_intervals = 30  
                    loop_index = 0
            ...
            for line in list_res.stdout.splitlines():
                ...
                if parts and uuid_pattern.match(parts[0]):
                    ...
                    try:
                        ...
                        master_content += f"## Analysis Segment\n```text\n{view_res.stdout.strip()}\n```\n\n"
