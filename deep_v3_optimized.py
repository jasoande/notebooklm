...
def execute_with_backoff(
    ...
    Args:
        ...
        input_data: Optional stdin data for subprocess
        max_attempts: Maximum number of retry attempts
        base_delay: Base delay for exponential backoff (seconds)
        cmd_timeout: Timeout for subprocess execution (seconds)
    ...
    # Raise exception to fail this specific pipeline thread gracefully without killing the orchestrator
    raise RuntimeError(error_msg)

# OPTIMIZED: Improved auth refresh with expiration handling
...
def update_dashboard(
    ...
    **kwargs: Any
    ...
    Uses DashboardManager if available, otherwise falls back to legacy dashboard.

    Args:
        client_token: Client identifier
        step: Current step description
        progress: Progress percentage (0-100)
        status: Status string (RUNNING, COMPLETE, FAILED, etc.)
        plan_link: Optional URL to account plan document
        **kwargs: Additional fields (e.g., notebook_id)
    """
    client_name = getattr(config, f"{client_token}_name", client_token)

    if DASHBOARD_MANAGER_AVAILABLE and dashboard:
        # Use modern DashboardManager (same as fast.py)
        dashboard.update(
            ...
            status=status,
            plan_link=plan_link,
            **kwargs  # Pass through notebook_id and other extras
        )
    else:
        # Legacy fallback
        with dashboard_lock:
            ...
            # Store additional kwargs (like notebook_id)
...
def ensure_notebook_exists(client_str: str, shared_auth_path: Path) -> Optional[Tuple[str, str, str]]:
    """
    Ensure NotebookLM workspace exists for client, creating if needed.

    Args:
        client_str: Client token identifier
        shared_auth_path: Path to shared authentication file

    Returns:
        Tuple of (client_str, client_name, notebook_id) or None if skipped

    Raises:
        Exception: If notebook creation or validation fails
    """
    # OPTIMIZED: Use persistent storage (no recreation)
    ...
    try:
        ...
        # Update dashboard with notebook_id so NotebookLM link appears
        ...
        raise e
    # NOTE: Storage file persists - no cleanup in finally block (optimization)
...
def ingest_client_sources(client_str, shared_auth_path, nb_id):
    ...
    try:
        ...
        # NOTE: Storage file persists - no cleanup (optimization)
...
def execute_ask_prompts_and_harvest(client_str, ask_prompt_files, shared_auth_path, nb_id):
    ...
    try:
        ...
        for p_idx, prompt_file in enumerate(ask_prompt_files):
            ...
            # CRITICAL FIX: Deep research has VERY strict rate limits
            # Add initial cooldown before first attempt
            if p_idx == 0:
            ...
            # OPTIMIZED: Use rate limiter instead of lock
            ...
            # CRITICAL FIX: Add cooldown AFTER each deep research
            ...
            if last_response_obj and last_response_obj.stdout:
                ...
                if discovered_citations:
                    ...
                    # OPTIMIZED: Use async URL validation with error handling
                    try:
...
def execute_pipeline(client_str, ask_files, chat_files, shared_auth_store):
    # Declare globals at top of function
...
def cleanup_old_session_storage():
    ...
    try:
        import time
        cleaned = 0
        cutoff_time = time.time() - 300  # Only delete files older than 5 minutes

        for pattern in [".storage_*.json", ".temp_ask_*", ".temp_chat_*"]:
            for f in SCRIPT_DIR.glob(pattern):
                try:
                    # Only delete if file is old (prevents deleting files from concurrent runs)
...
def ensure_authentication():
    """
    Ensure authentication is available, either from shared storage or default location.
    Validates that auth is still valid before proceeding.
    Returns the path to the auth file to use.
    """
    shared_auth_store = SCRIPT_DIR / ".shared_auth.json"
    default_auth = Path.home() / ".notebooklm" / "profiles" / "default" / "storage_state.json"

    # Function to test if auth is valid
    ...
    # Check if shared auth exists and is valid
    if shared_auth_store.exists():
        ...
        if test_auth(shared_auth_store):
            ...
            shared_auth_store.unlink()  # Remove expired auth

    # Check if default auth exists and is valid
...
def main():
    ...
    # PRODUCTION: Clean up old session files
    cleanup_old_session_storage()

    # OPTIMIZED: Smart authentication handling
    shared_auth_store = ensure_authentication()

    # CRITICAL: Start background auth refresh to prevent token expiration
    ...
    # Start UI thread only if using legacy dashboard
    if not DASHBOARD_MANAGER_AVAILABLE or not dashboard:
